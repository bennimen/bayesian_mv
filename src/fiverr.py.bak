...
def probability_F_beats_ABC(alpha_A, beta_A, alpha_B, beta_B, alpha_C, beta_C, alpha_D, beta_D, alpha_E, beta_E, alpha_F, beta_F):
    total = 0.0
    for i in range(0, (alpha_A - 1)):
        for j in range(0, (alpha_B - 1)):
            for k in range(0, (alpha_C - 1)):
                for l in range(0, (alpha_D - 1)):
                    for m in range(0, (alpha_E - 1)):

                        total += math.exp(bln(alpha_D + i + j + k + l + m, beta_A + beta_B + beta_C + beta_D + beta_E + beta_F)
